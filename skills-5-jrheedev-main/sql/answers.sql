...
SELECT SUM(num_cupcakes) FROM orders WHERE processed is FALSE;
...
SELECT cupcakes.name, SUM(orders.num_cupcakes) FROM orders RIGHT JOIN cupcakes on cupcakes.id = orders.cupcake_id GROUP BY cupcakes.name ORDER by cupcakes.name ASC, SUM(orders.num_cupcakes);
...
SELECT email FROM customers LEFT JOIN orders on customers.id = orders.customer_id GROUP BY email having SUM(num_cupcakes) > 100 ORDER BY email DESC; 
...
SELECT DISTINCT fname, lname, email FROM customers JOIN  orders ON customers.id = orders.customer_id WHERE processed = 'True' AND  cupcake_id = (SELECT id FROM cupcakes WHERE name = 'funfetti');